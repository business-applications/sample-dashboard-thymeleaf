...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-dashboard-thymeleaf-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-dashboard-thymeleaf-service apps/sample-dashboard-thymeleaf-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-dashboard-thymeleaf-service:1.0-SNAPSHOT
    oc expose svc/sample-dashboard-thymeleaf-service
else

	echo "Launching the application locally..."
	pattern="sample-dashboard-thymeleaf-service"
	files=( $pattern )
	cd ${files[0]}
