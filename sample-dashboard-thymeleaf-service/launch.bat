...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sample-dashboard-thymeleaf-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sample-dashboard-thymeleaf-service apps/sample-dashboard-thymeleaf-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sample-dashboard-thymeleaf-service:1.0-SNAPSHOT
		call oc expose svc/sample-dashboard-thymeleaf-service
	) else (
